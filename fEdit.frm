...
Select Case Step
  ...
  Case 6 'Finish
    ...
    Step = 1
    OverlapEvent slot, new_event
    UpdateList
    lstEvents.ListIndex = slot
...
Private Sub lstEvents_Click()
    ShowEvent Events(lstEvents.ListIndex)
    slot = lstEvents.ListIndex
    PrepareStep
End Sub

Public Sub txtAlert_Change()
...
Private Sub ShowEvent(Evnt As Evt)
    If Evnt.alert Then chkAlert.Value = 1 Else chkAlert.Value = 0
    optAnnually.Value = Evnt.annual
    optByDOW.Value = Evnt.by_day_of_week
    optByDate.Value = Not Evnt.by_day_of_week
    txtDay.Text = Evnt.day
    cboDOW.ListIndex = Evnt.day_of_week - 1
    txtAlert.Text = Evnt.days_ahead_to_alert
    txtDes.Text = Evnt.description
    cboMonth.ListIndex = Evnt.Month - 1
    optMonthly.Value = Evnt.monthly
    optOnce.Value = Evnt.once
    txtTitle.Text = Evnt.Title
    txtWeek.Text = Evnt.Week
    optWeekly.Value = Evnt.weekly
    txtYear.Text = Evnt.Year
